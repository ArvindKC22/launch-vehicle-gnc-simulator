...
StateDerivative derivative(const State& x, const VehicleParameters& p, const Inputs& u) {
    ...
    if (!p.motor.empty() && xn.propellant_mass > 0.0) {
        const MotorPoint first{p.motor.front().time_s, p.motor.front().vacuum_thrust_n, p.motor.front().mass_flow_kgps};
        std::vector<TablePoint> thrust_table, flow_table;
        thrust_table.reserve(p.motor.size()); flow_table.reserve(p.motor.size());
        for (const auto& m : p.motor) { thrust_table.push_back({m.time_s, m.vacuum_thrust_n}); flow_table.push_back({m.time_s, m.mass_flow_kgps}); }
        const double thrust_vac = interp(thrust_table, u.time_s, first.vacuum_thrust_n);
        mdot = std::max(0.0, interp(flow_table, u.time_s, first.mass_flow_kgps));
        const double thrust = thrust_vac - p.nozzle_exit_area_m2 * env.pressure_pa;
        const Eigen::AngleAxisd tvc_yaw(u.tvc_yaw_rad, Vec3::UnitZ());
        const Eigen::AngleAxisd tvc_pitch(u.tvc_pitch_rad, Vec3::UnitY());
